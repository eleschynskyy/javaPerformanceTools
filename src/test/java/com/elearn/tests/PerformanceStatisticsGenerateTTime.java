...
public class PerformanceStatisticsGenerateTTime {
...
//	private static String PATH = "D:/Xyleme/performance/products/bcp/tests/";
	private static String FILENAME = "sps_perf_script_CLOUD-1453305120-complete.jtl";
		...
		try {
			...
			if (keys != null) {
				...
				for(String label: errors.keySet()){
					...
					errorWriter.write(label + "|" + errors.get(label).get(0) + "|" + errors.get(label).get(1) + "|" + pString + "\n");
				}
				errorWriter.close();
			}
			reader.close();
			...
			outputWriter.write("REQUEST,MEDIAN,Q1,Q3,LOWER,UPPER,LFENCE%,UFENCE%,MIN,MAX,AVG,STD,ERRORS%\n");
...
//				errorPercentage);
	...
	private static void generateTTime(){
		File file = new File(OUTPUT_PATH + "output.txt");
		try{
			BufferedReader reader = new BufferedReader(new FileReader(file));
			String[] keys = reader.readLine().split(",");
			if (keys != null) {
				String[] dataParts;
				String line;
				File tTimeFile = new File(OUTPUT_PATH + "_apdex_t_time.txt");
				if (tTimeFile.exists()) {
					tTimeFile.delete();
				}
				...
				while ((line = reader.readLine()) != null) {
					...
					float elapsed = Float.parseFloat(row.get("AVG"));
					int tTime = Math.round(elapsed + elapsed * APDEX_T_TIME_RATE / 100);
					tTimeWriter.write("_" + row.get("REQUEST") + ".txt," + Math.round(elapsed) + ".0," + tTime + ".0\n");
				}
				tTimeWriter.close();
			}
			reader.close();
