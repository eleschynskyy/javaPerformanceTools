...
public class ExtractReviewSessionInfo {
	...
	private static String OUTPUT_PATH = PATH + TEST_FOLDER + "/TEST_" + TEST_NUMBER + "/graph/data/";
					...
					if(child.getFirstChild().getTextContent().startsWith("Container")){
						for(int j = 1; j <= child.getChildNodes().getLength() - 1; j++){
							...
							for(int k = 0; k <= child.getChildNodes().item(j).getAttributes().getLength() - 1; k++){
								guidMatcher = guidPattern.matcher(child.getChildNodes().item(j).getAttributes().item(k) + "");
								if(guidMatcher.matches()){
									containerGuidWriter.write(guidMatcher.group(1) + "\n");
								}
							}
							for(int k = 0; k <= child.getChildNodes().item(j).getFirstChild().getChildNodes().getLength() - 1; k++){
								if(child.getChildNodes().item(j).getFirstChild().getChildNodes().item(k).getNodeName().equals("ParaBlock")){
									for(int t = 0; t <= child.getChildNodes().item(j).getFirstChild().getChildNodes().item(k).getAttributes().getLength() - 1; t++){
										guidMatcher = guidPattern.matcher(child.getChildNodes().item(j).getFirstChild().getChildNodes().item(k).getAttributes().item(t) + "");
										if(guidMatcher.matches()){
